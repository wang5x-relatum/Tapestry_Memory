...
本仓库处于阶段 2：独立记忆库的本地核心实现。这不是已发布的包，也不是已通过产物验收的版本。单元与契约验收必须针对本地实现报告，并与文档检查分开；本页不宣称测试已通过。wheel/sdist 安装及产物验收推迟至阶段 2.4。`0.1.0` 是目标版本，不是已发布版本。包名可用性、许可证、权利主体及公开托管地址仍需发布前确认。
